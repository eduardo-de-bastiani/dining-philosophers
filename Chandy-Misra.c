...
void *philosopher(void *tid) {
    ...
    while (*(int*)tid != -1) {
        strcpy(state[thisID], "Thinking");
        usleep(random() % SLEEP_MAX);

        strcpy(state[thisID], "Waiting");
        ...
        //and the first fork was last used by this philosopher
    ...
    strcpy(state[thisID], "Terminated");
    pthread_exit(NULL);
}
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < N; i++) {
        ...
        state[i] = (char*)malloc(sizeof(char) * 20);  // Allocate space for state strings
        strcpy(state[i], "Waiting");
        heldBy[i] = -1;
        prevUsedBy[i] = i;
    }
    ...
    //cancel and wait for all philosopher threads to terminate
    for (i = 0; i < N; i++)
    ...
    for (i = 0; i < N; i++) {
        ...
        free(state[i]);  // Free allocated memory for state strings
    }
